...
class ListadoPersonalVehiculoController extends GetxController
  ...
  final GetPersonalVehiculoByTemporadaUseCase _getPersonalVehiculoByTemporadaUseCase;
  ...
  Future<void> getPersonalVehiculo() async {
    personalSeleccionado =
        await _getAllPersonalVehiculoUseCase.execute('personal_vehiculo_$key');
  }

  Future<void> getPersonalVehiculoByTemporada() async {
    enBase =
        await _getPersonalVehiculoByTemporadaUseCase.execute(idtemporada);
  }
    ...
    basicDialog(
      ...
      () async {
        ...
        await _deletePersonalVehiculoUseCase.execute(
            'personal_vehiculo_$key', personalSeleccionado[index].key);
  ...
  Future<void> setCodeBar(dynamic barcode, [bool byLector = false]) async {
    ...
    if (barcode != null && barcode != '-1') {
      for (var element in otrosVehiculoTemporada) {
        Box<PersonalVehiculoEntity> personalOtra=await Hive.openBox('personal_vehiculo_${element.key}');
      ...
      int indexEnBase = enBase
          .indexWhere((e) => e.nrodocumento == barcode.toString());
      if (indexEnBase != -1) {
        update(['seleccionados', 'personal_seleccionado_pv', 'listado_v']);
        byLector
      ...
      if (indexEncontrado != -1) {
        ...
        byLector
            ...
            : await _showNotification(false, 'Personal registrado anteriormente.');
        return;
      }
      ...
      if (index != -1) {
        ...
        await _createPersonalVehiculoUseCase.execute(
            'personal_vehiculo_$key', p);
        update(['seleccionados', 'personal_seleccionado_pv', 'listado_v']);
      } else {
        byLector
